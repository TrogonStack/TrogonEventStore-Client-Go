...
func (suite *DeleteTestSuite) TestCanDeleteStream() {
	...
	opts := trogoneventstore.DeleteStreamOptions{
		StreamState: trogoneventstore.Revision(0),
	}

	streamId := suite.fixture.NewStreamId()
	_, err := client.AppendToStream(context.Background(), streamId, trogoneventstore.AppendToStreamOptions{}, suite.fixture.CreateTestEvent())
...
func (suite *DeleteTestSuite) TestCanTombstoneStream() {
	...
	streamId := suite.fixture.NewStreamId()
	_, err := client.AppendToStream(context.Background(), streamId, trogoneventstore.AppendToStreamOptions{}, suite.fixture.CreateTestEvent())
	deleteResult, err := client.TombstoneStream(context.Background(), streamId, trogoneventstore.TombstoneStreamOptions{
		StreamState: trogoneventstore.Revision(0),
	})
	if err != nil {
		suite.T().Fatalf("Unexpected failure %+v", err)
...
func (suite *DeleteTestSuite) TestDetectStreamDeleted() {
	...
	_, err = client.TombstoneStream(context.Background(), streamId, trogoneventstore.TombstoneStreamOptions{})
	require.NoError(suite.T(), err)
	...
	require.Equal(suite.T(), trogoneventstore.ErrorCodeStreamDeleted, serverError.Code())
}
