...
import (
	...
	gossipApi "github.com/TrogonStack/TrogonEventStore-Client-Go/protos/trogoneventstore/protocols/v1/gossip"
	server_features "github.com/TrogonStack/TrogonEventStore-Client-Go/protos/trogoneventstore/protocols/v1/serverfeatures"
...
func getSupportedMethods(ctx context.Context, conf *Configuration, conn *grpc.ClientConn) (*serverInfo, error) {
	...
	for idx, value := range strings.Split(methods.EventStoreServerVersion, ".") {
		...
		if err != nil {
			return nil, fmt.Errorf("invalid TrogonEventStore server version format: %w", err)
		}
